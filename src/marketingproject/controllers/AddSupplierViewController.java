...
public class AddSupplierViewController implements Initializable {
    ...
    @FXML
    private void btn_add_supplier_button_event(ActionEvent event) {
        String company_name = txt_company_name.getText();
        
        if(company_name.isEmpty()){
            lbl_status_message.setText("Please write a supplier name...");
            lbl_status_message.setTextFill(Color.RED);
            return;
        }
        
        if(connection.AddSupplier(company_name)){
            lbl_status_message.setText("Supplier add success ...");
            lbl_status_message.setTextFill(Color.GREEN);
            txt_company_name.setText("");
        }
        else{
            lbl_status_message.setText("Supplier add error ...");
            lbl_status_message.setTextFill(Color.RED);
        }
